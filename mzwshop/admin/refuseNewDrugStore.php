...
<?php
	...
	$link=connectAndSelectDB();//连接数据库
	$sql="select * from t_store where store_id ='{$mzwid}'";
	$row=fetchOne($sql,$link);
	$link=connectAndSelectDB();//连接数据库
	$sql="update t_store set store_check='3' where store_id='{$mzwid}'";
	$rows=update($sql);
	$sql="update t_store set store_permit='0' where store_id='{$mzwid}'";
	$rows=update($sql);
	//获取最大消息编号
	$sql="select max(mes_id) from t_message";
	$row=fetchOne($sql,$link);
	$index=$row[0]+1;
	$sql="insert into t_message values('a','000000','s','{$mzwid}','{$index}','您的开店请求被拒绝')";
	$rows=insert($sql);
